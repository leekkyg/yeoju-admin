...
import { Plus, Trash2, Edit, Image as ImageIcon, Calendar, ArrowLeft, Clock } from "lucide-react";
...
export default function MainBannerPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const fileName = `ads/main-banner/${Date.now()}_${file.name}`;
      ...
      if (editingId) {
        const { error } = await supabase
          ...
          .update({
            ...
            slide_duration: parseInt(slideDuration),
          })
          .eq("id", editingId);
  ...
  const handleEdit = (ad: Ad) => {
    ...
    setSlideDuration(ad.slide_duration?.toString() || "3");
    setShowForm(true);
  };
      ...
      <main className="flex-1 p-8 max-w-[631px] mx-auto w-full">
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <h1 className="text-3xl font-bold text-slate-900">메인 배너 관리</h1>
            </div>
            <p className="text-slate-500">홈 상단 슬라이더 배너</p>
          </div>
          <Button
        ...
        {showForm && (
          <Card className="mb-6">
            <CardContent className="p-6 space-y-4">
              <h3 className="text-lg font-bold">{editingId ? "배너 수정" : "배너 추가"}</h3>
              ...
              <div>
                <Label>배너 이미지 * (권장: 1000 x 300px)</Label>
                <input
                  ref={fileInputRef}
                  type="file"
                  accept="image/*"
              ...
              <div>
                ...
                <Select value={slideDuration} onValueChange={setSlideDuration}>
                  <SelectTrigger className="mt-2">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="2">2초</SelectItem>
                    <SelectItem value="3">3초 (권장)</SelectItem>
        ...
        {loading ? (
          ...
          <Card className="p-12 text-center">
            ...
            <Button onClick={() => setShowForm(true)}>첫 배너 등록하기</Button>
          </Card>
        ) : (
            ...
            {ads.map((ad) => {
              ...
              return (
                <Card key={ad.id} className={!active ? "opacity-50" : ""}>
                  <div className="aspect-[10/3] bg-slate-100 relative">
                    <img
                      src={ad.image_url}
                      alt={ad.title}
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-500 mb-2">
                      ...
                      <span>{ad.slide_duration || 3}초 전환</span>
                    </div>
                    {ad.target_url && (
                      <a
                        href={ad.target_url}
